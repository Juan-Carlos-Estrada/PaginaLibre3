...
public class Conexion {
    ...
    private Conexion() {
        ...
        try (InputStream in = getClass().getResourceAsStream(CONFIG_FILE)) {
            ...
            // cargamos el archivo dentro de la clase properties
            config.load(in);
        } catch (IOException e) {
            throw new IllegalStateException("Error al leer " + CONFIG_FILE, e);
        }

        // llenamos nuestras variables finales con los datos de db.properties
        this.url = config.getProperty("db.url");
        this.user = config.getProperty("db.user");
        this.password = config.getProperty("db.password");

        // comprobacion de datos del properties nulos para cada atributo
    ...
    // Metodo publico estatico para obtener la unica instancia del Gestor
    ...
    // Metodo para entregar una conexion fresca cada vez que se pida
